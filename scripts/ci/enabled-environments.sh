...
#
# .github/environments.json lists the environments the team tools run in:
#
#   ["development", "production"]
#
# Any one, two or all three of development, staging and production, and only
# environments core runs too (the tools have no platform where core has none).
# Folders of the others stay in the repository, ignored, so turning one on
# later is one line here (and scripts/init-tools.sh). Every workflow asks this
# script, so nothing is planned, applied or started for an environment the
# tools do not run.
#
# Usage:
...
case "${1:-}" in
  ...
  --check)
    ...
    if ! jq -e --arg e "${ENVIRONMENT}" 'index($e) != null' <<< "${ENABLED}" >/dev/null; then
      ...
      echo "       To run it, add it to .github/environments.json (scripts/init-tools.sh --environments)." >&2
      exit 1
    fi
    ;;
  --filter)
